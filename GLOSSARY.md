...
| **Narration** | The human-readable text on a step or scenario name. Carried both as a flat rendered string and, when authored via a t-string or `pytest_given.Template`, as a structured `list[NarrationPart]` (a sequence of `NarrationLiteral` / `NarrationValue` / `NarrationPlaceholder` pieces). The structured form lets the templatizer and renderer treat parametrize-bound values specially without regex tricks. |
