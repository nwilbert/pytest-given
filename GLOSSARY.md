...
| **Deferred term** | A term obtained before its kind is known — the `DeferredTermHandle` returned by the code glossary (`g('foo')` declare-or-get, `g['foo']` get-only) and by a *file glossary* (`g['Guest']`). It is the one `TermHandle` type every term is handed as, refined only by what defers: the kind stays `None` until *kind inference* runs. |
