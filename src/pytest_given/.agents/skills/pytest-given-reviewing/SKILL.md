...
- **Raises** — the `then` of an expected raise may translate the `match=`-pinned message into domain terms, but must not add claims the message doesn't carry. Judge against the pin, not the implementation. A `then` promising that the message names the offender, offers a hint, or carries a file:line is under-pinned when the regex would also pass on a message lacking that detail — `match='Gues'` matches the echoed bad input rather than the did-you-mean suggestion. The implementation may include the detail today; the narrated claim is unverified either way, and can regress silently. An alternation pin (`match=r'odd|dangling|ends'`) is only as strong as its weakest branch, so siblings narrating *different* refusals all pass on one generic message. Pin what tells this refusal from its neighbours.
...
- An **uncovered story activity** is a gap worth noting — unless the declaration site marks it deliberate (a story maps the whole flow, human activities included). Coverage is matched per step: one step's term refs must include all of an activity's terms. The Stories tab shows it, but that is HTML and `--given-md` has no Stories section — read the JSON report's `coverage[]` instead; the query, and the full rule for saying *why* an activity is uncovered, are in [references/story-coverage.md](references/story-coverage.md).
