...
- **`attach(label, content)`** — attach data to the current step. `label` is a plain `str` (a t-string or `Template` raises); strings stored verbatim, other types JSON-serialized.
