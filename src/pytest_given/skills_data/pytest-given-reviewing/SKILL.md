...
- An **uncovered story activity** is a gap worth noting. Coverage is matched per step: one step's term refs must include all of an activity's terms. The Stories tab shows it, but that is HTML and `--given-md` has no Stories section — the full rule and a ready-to-run query over the JSON report are in [references/story-coverage.md](references/story-coverage.md).
