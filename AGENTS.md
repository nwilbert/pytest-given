...
- The `examples` and `self_report` sessions run with `--given-phase-check=warn`, so regenerating a report prints an "incomplete scenarios" summary if any `@scenario` slipped to two phases (see [Phase check](README.md#phase-check)). The backend suite is currently clean; keep it that way. Audit locally any time with `uv run pytest tests/unit --given-phase-check=warn`. A scenario that is *honestly* two-phase (a static-property `given`+`then`, a pure `when_then` raise — see the GWT rules under [Writing self-report scenarios](#writing-self-report-scenarios)) belongs on the `given_phase_check_ignore` list, not forced to three phases.
