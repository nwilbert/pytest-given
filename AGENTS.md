...
- The `self_report` session runs with `--given-phase-check=error` (see [Phase check](README.md#phase-check)): the backend suite has no intentional failures, so a scenario that slips to two phases **fails report regeneration** — a real gate. The `examples` session runs `warn` instead, because its intentional failures already return a tolerated exit 1 (`success_codes=[0, 1]`) that would mask an error-mode phase failure; there the printed "incomplete scenarios" summary is the signal. Keep the backend suite clean; audit locally with `uv run pytest tests/unit --given-phase-check=warn`. A scenario that is *honestly* two-phase (a static-property `given`+`then`, a pure `when_then` raise — see the GWT rules under [Writing self-report scenarios](#writing-self-report-scenarios)) belongs on the `given_phase_check_ignore` list, not forced to three phases.
