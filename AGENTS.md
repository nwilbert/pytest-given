...
- `src/pytest_given/plugin.py` — pytest hooks, parametrized test grouping, structural templatize, scenario-source capture from `item.location`. Top-level orchestrator; allowed to import from all four subpackages.
...
- **Verify the narration reads truthfully before committing.** Render the touched scenarios (`uv run pytest <selection> --given-md`) and read the output as a spec: every step text must be something its body actually does. Narration is auditable, not verified — the lint catches structural lies, but nothing mechanically checks semantic truth — so a step whose text overstates its body is a bug of the same severity as a wrong assertion.
...
- Regeneration (`uv run nox -s self_report`), narration-lint gating, and the commit-noise rules live under [Quality gates](#quality-gates).
