...
- Subpackage boundaries: `src/pytest_given/` is split into three subpackages with a strict dependency direction. `model/` is the leaf; `capture/` and `report/` both depend on `model/`; they do not depend on each other. `plugin.py` sits at the top level as the orchestrator and is allowed to import from all three. Cross-subpackage code goes through the subpackage root (`from ..model import Scenario`), never reaches into submodules (`from ..model.schema import Scenario` is forbidden). Within a subpackage, use single-dot relative imports (`from .schema import Scenario`). Both rules — direction and submodule boundary — are enforced by ruff `TID251` + `TID253` (see `pyproject.toml`). Tests are exempt and may import internals directly.
