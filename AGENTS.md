...
- Both regeneration sessions run the narration lint (`--given-lint=true`; see the [design spec](docs/specs/proposed/2026-07-05-narration-lint-design.md) — v1 ships `empty-step` and `then-without-check`, both error-level): in `self_report` the backend suite has no intentional failures, so an error finding **fails report regeneration** — a real gate. The `examples` session's intentional failures already return a tolerated exit 1 (`success_codes=[0, 1]`) that masks the lint exit code; there the printed "narration lint" summary is the signal. Keep the backend suite lint-clean; a step the lint mis-flags belongs on the `given_lint_ignore` list, whose entries must each suppress a finding (stale entries fail the run). The standalone phase check (`--given-phase-check`, see [Phase check](README.md#phase-check)) still exists, is off by default, and folds into the lint as the `missing-phase` rule in a later step; audit locally with `uv run pytest tests/unit --given-phase-check=warn`. A scenario that is *honestly* two-phase (a static-property `given`+`then`, a pure `when_then` raise — see the GWT rules under [Writing self-report scenarios](#writing-self-report-scenarios)) belongs on the `given_phase_check_ignore` list, not forced to three phases.
...
When decorating a backend test with `@scenario`, the goal is a report that reads as a truthful behavioural spec.

The narration rules below are **portable** — they apply to any project using pytest-given and are the seed of the planned downstream skill (see TODO.md). The [mechanics subsection](#self-report-mechanics-this-repo) is specific to this repo's self-report.

### Narration rules (portable)
...
- **Narrate in glossary vocabulary.** Reference glossary terms via term handles inside t-string step text (in this repo: `pg['Term']`, e.g. `t'a {pg["File glossary"]} loaded from a file'`). Term refs are what power the Glossary tab's per-term filter, and they render as pills.
...
- **Verify the narration reads truthfully before committing.** Render the touched scenarios (`uv run pytest <selection> --given-md`) and read the output as a spec: every step text must be something its body actually does. Narration is auditable, not verified — nothing mechanically compares text to body — so a step whose text overstates its body is a bug of the same severity as a wrong assertion.
...
- Regeneration (`uv run nox -s self_report`), phase-check gating, and the commit-noise rules live under [Quality gates](#quality-gates).
- **Review the `.md` diff after regenerating.** `examples/self-report/self-report.md` is deterministic (no `commit_sha` / `duration_ms`), so its diff is the behavioural delta of your change in prose — the primary thing to sanity-check before committing regenerated outputs. The same applies to the `examples/*/*.md` files after `uv run nox -s examples`.
