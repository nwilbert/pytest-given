...
2. **Render the spec.** `pytest <selection> --given-md` runs the selected tests and prints a Markdown spec of every scenario to stdout, fenced between `<!-- pytest-given:md:start -->` and `:end`. Each scenario heading carries a ✓ / ✗ / ○ status glyph, term refs render as `«term»`, and the line under the heading is a ``file.py:line::test_name`` anchor followed by the scenario's tags. A skipped scenario adds ` · skipped` and its reason; a grouped parametrized one adds ` · N cases` and renders its parameter table below the steps, one row per case with its own glyph. Select with pytest's own args (`-k`, node ids, `--lf`); the renderer narrates whatever ran.
